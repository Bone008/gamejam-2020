...
public class SetPivot : EditorWindow {
    ...
    [MenuItem ("Util/Set Pivot")] //Place the Set Pivot menu item in the GameObject menu
    static void Init () {
	...
	void OnGUI() {
		if(obj) {
			if(mesh) {
                ...
                if (GUILayout.Button("Create FilterMesh"))
                {
                    string filePath = EditorUtility.SaveFilePanelInProject("Mesh Filter Name", "My Mesh", "asset", "");
                    if (filePath == "") return;
	...
	void UpdatePivot() { 
		...
		obj.transform.localPosition -= Vector3.Scale(diff, obj.transform.localScale); //Move object position by taking localScale into account
		...
		if(col) {
			if(col is BoxCollider) {
                ...
                // [MOD] added MeshCollider
            else if (col is MeshCollider)
            {
                ((MeshCollider)col).sharedMesh = null;
                ((MeshCollider)col).sharedMesh = mesh;
            }
